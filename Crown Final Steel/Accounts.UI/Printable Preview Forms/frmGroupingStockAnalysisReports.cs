...
{
    public partial class frmGroupingStockAnalysisReports : MetroForm
    {
        #region Variables
        ...
        public frmGroupingStockAnalysisReports()
        {
            InitializeComponent();
        }
        private void frmGroupingStockAnalysisReports_Load(object sender, EventArgs e)
        {
            PrintReport();
        }
        private void frmGroupingStockAnalysisReports_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        private void PrintReport()
        {
            ...
            if(!IsDateWise)
                RptDocument.Load("..//..//Stock Reports/rptTotalProductsGroupingStockReport.rpt");
            else
                RptDocument.Load("..//..//Stock Reports/rptDateWiseTotalProductsGroupingStockReport.rpt");
            ...
            foreach (ParameterFieldDefinition def in crParamFieldDefinitions)
            {
                ...
                if (def.ReportName == "")
                {
                    ...
                    if (def.ParameterFieldName == "@IdProject")
                    {
                    ...
                    if (def.ParameterFieldName == "@BookNo")
                    {
                        ParameterDiscreteValue crParamDiscreteValue = new ParameterDiscreteValue();
                        ParameterValues crCurrentValues = def.CurrentValues;
                    ...
                    else if (def.ParameterFieldName == "@StartDate")
                    {
                        ...
                        crParamDiscreteValue.Value = StartDate; //"{" + Operations.IdCompany + "}";
                        crCurrentValues.Add(crParamDiscreteValue);
                        def.ApplyCurrentValues(crCurrentValues);
                    }
                    else if (def.ParameterFieldName == "@EndDate")
                    {
                        ...
                        crParamDiscreteValue.Value = EndDate; //"{" + Operations.IdCompany + "}";
                        crCurrentValues.Add(crParamDiscreteValue);
                        def.ApplyCurrentValues(crCurrentValues);
                    }
